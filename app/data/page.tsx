...
interface FormEntry {
  id: string; name: string; type: string | null; platform: string | null
  link: string | null; responses: number | null; date: string | null
...
/* ══════════════════════════════════════════════════════════
   FORMS SECTION
══════════════════════════════════════════════════════════ */
const FORM_EMPTY = { name: '', type: '', platform: '', link: '', responses: '', date: '', notes: '' }

function FormsSection() {
  const [rows, setRows] = useState<FormEntry[] | null>(null)
  const [selected, setSelected] = useState<FormEntry | null>(null)
  const [showAdd, setShowAdd] = useState(false)
  const [form, setForm] = useState(FORM_EMPTY)
  const [saving, setSaving] = useState(false)
  const [editing, setEditing] = useState(false)
  const [editForm, setEditForm] = useState<Omit<Partial<FormEntry>, 'responses'> & { responses?: string }>({})
  const [editSaving, setEditSaving] = useState(false)

  useEffect(() => {
    supabase.from('data_forms').select('*').order('created_at', { ascending: false })
      .then(({ data }) => setRows((data as FormEntry[]) ?? []))
  }, [])

  async function submit(e: React.FormEvent) {
    e.preventDefault(); if (!form.name) return
    setSaving(true)
    const { data } = await supabase.from('data_forms').insert({
      name: form.name.trim(), type: form.type.trim() || null, platform: form.platform.trim() || null,
      link: form.link.trim() || null, responses: form.responses ? parseInt(form.responses) : null,
      date: form.date || null, notes: form.notes.trim() || null,
    }).select().single()
    if (data) { setRows(prev => [data as FormEntry, ...(prev ?? [])]); setSelected(data as FormEntry) }
    setForm(FORM_EMPTY); setShowAdd(false); setSaving(false)
  }

  async function saveEdit() {
    if (!selected) return; setEditSaving(true)
    const { data } = await supabase.from('data_forms').update({
      name: editForm.name, type: editForm.type || null, platform: editForm.platform || null,
      link: editForm.link || null, responses: editForm.responses ? parseInt(editForm.responses) : null,
      date: editForm.date || null, notes: editForm.notes || null,
    }).eq('id', selected.id).select().single()
    if (data) { const u = data as FormEntry; setSelected(u); setRows(prev => prev?.map(r => r.id === selected.id ? u : r) ?? null) }
    setEditing(false); setEditSaving(false)
  }

  async function del(id: string) {
    if (!confirm('Delete this entry?')) return
    await supabase.from('data_forms').delete().eq('id', id)
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <button onClick={() => { setForm(FORM_EMPTY); setShowAdd(true) }}
      ...
      {showAdd && (
        <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-5 mb-5">
          <h3 className="text-sm font-semibold text-stone-700 mb-3">New Form Entry</h3>
          <form onSubmit={submit} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><label className="text-xs text-stone-400 mb-1 block">Form Name *</label>
                <input required className={inputCls} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
              <div><label className="text-xs text-stone-400 mb-1 block">Type</label>
                <input className={inputCls} placeholder="e.g. Interest, Contact, Survey" value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} /></div>
              <div><label className="text-xs text-stone-400 mb-1 block">Platform</label>
                <input className={inputCls} placeholder="e.g. Google Forms, Typeform" value={form.platform} onChange={e => setForm(f => ({ ...f, platform: e.target.value }))} /></div>
              <div><label className="text-xs text-stone-400 mb-1 block">Responses</label>
                <input type="number" className={inputCls} value={form.responses} onChange={e => setForm(f => ({ ...f, responses: e.target.value }))} /></div>
              ...
              <div className="col-span-2"><label className="text-xs text-stone-400 mb-1 block">Link</label>
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={saving || !form.name} className="px-4 py-1.5 text-white text-sm rounded-lg disabled:opacity-40 font-medium" style={goldBtn}>{saving ? 'Saving…' : 'Add'}</button>
      ...
      {rows === null ? <div className="text-center py-16 text-stone-400 text-sm">Loading…</div> : (
        <div className="grid gap-5 grid-cols-[1fr_360px]">
          <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
            {rows.length === 0 ? <div className="text-center py-16 text-stone-400 text-sm">No forms logged yet.</div> : (
              <table className="w-full text-sm">
                <thead><tr className="border-b border-stone-100">
                  ...
                  <th className="px-4 py-3 text-xs font-semibold text-stone-400 uppercase tracking-wider text-left">Type</th>
                  <th className="px-4 py-3 text-xs font-semibold text-stone-400 uppercase tracking-wider text-right">Responses</th>
                ...
                <tbody>{rows.map(r => (
                  <tr key={r.id} onClick={() => { setSelected(prev => prev?.id === r.id ? null : r); setEditing(false) }}
                    ...
                    <td className="px-4 py-3 text-stone-500">{r.type ?? <span className="text-stone-300">—</span>}</td>
                    <td className="px-4 py-3 text-right font-medium text-stone-700">{r.responses ?? <span className="text-stone-300">—</span>}</td>
                    <td className="px-4 py-3 text-right text-stone-400 text-xs">{r.date ? fmtDate(r.date) : '—'}</td>
                  </tr>
                ))}</tbody>
              </table>
            )}
            <div className="px-4 py-2.5 text-xs text-stone-400 border-t border-stone-100">{rows.length} form{rows.length !== 1 ? 's' : ''}</div>
          </div>
          {selected ? (
            <DetailPanel onClose={() => setSelected(null)}>
              <div className="flex items-start justify-between mb-4">
                <h2 className="font-bold text-stone-800 text-base leading-snug pr-2">{selected.name}</h2>
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => { setEditForm({ ...selected, responses: selected.responses != null ? String(selected.responses) : '' }); setEditing(true) }}
              ...
              {editing ? (
                <div className="space-y-3">
                  <div><label className="text-[10px] text-stone-400 uppercase tracking-wide mb-1 block">Name</label>
                    <input className={inputCls} value={editForm.name ?? ''} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} /></div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><label className="text-[10px] text-stone-400 uppercase tracking-wide mb-1 block">Type</label>
                      <input className={inputCls} value={editForm.type ?? ''} onChange={e => setEditForm(f => ({ ...f, type: e.target.value }))} /></div>
                    ...
                    <div><label className="text-[10px] text-stone-400 uppercase tracking-wide mb-1 block">Responses</label>
                      <input type="number" className={inputCls} value={editForm.responses ?? ''} onChange={e => setEditForm(f => ({ ...f, responses: e.target.value }))} /></div>
                ...
                <div className="space-y-3">
                  ...
                  <Field label="Responses" value={selected.responses} />
